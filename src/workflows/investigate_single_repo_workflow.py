...
class InvestigateSingleRepoWorkflow:
    ...
    def __init__(self) -> None:
        ...
        self._investigation_progress: str | None = None
        ...
        health_check_result = await workflow.execute_activity(
            ...
            retry_policy=RetryPolicy(
                ...
                maximum_interval=timedelta(
                    seconds=10
                ),  # Keep at 10 seconds to maintain consistent intervals
    ...
    async def _analyze_repository_structure(self, repo_path: str) -> dict[str, Any]:
        ...
        return cast(dict[str, Any], structure_result)

    async def _get_prompts_config(
        self, repo_path: str, repo_type: str, repo_url: str
    ) -> PromptsConfigResult:
    ...
    async def _read_and_cache_dependencies(self, repo_path: str) -> dict[str, Any]:
    ...
    async def _process_analysis_steps(
        ...
        step_results: dict[str, str | None] = {}  # Maps step names to result reference keys
        ...
        for step in processing_order:
            ...
            claude_input = AnalyzeWithClaudeInput(
                ...
                config_overrides=(
                    ClaudeConfigOverrides(**config_overrides.model_dump())
                    if config_overrides
                    else None
                ),
        ...
        try:
            ...
            save_metadata_input = SaveMetadataInput(
                ...
                analysis_summary={
                    ...
                    "architecture_hub_status": (
                        investigation_result.architecture_hub.get("status", "unknown")
                        if investigation_result.architecture_hub
                        else "unknown"
                    ),
                },
                prompt_versions=prompt_versions or {},
    ...
    async def run(self, request: InvestigateSingleRepoRequest) -> InvestigateSingleRepoResult:
        ...
        logger.info("🔍 WORKFLOW: Loading prompts configuration early for cache comparison")
        ...
        cache_check_result = await self._check_cache(
            repo_name, repo_url, repo_path, prompt_versions
        )
        ...
        logger.info("📝 WORKFLOW: Reusing prompts configuration loaded earlier")
        ...
        analysis_result = await self._process_analysis_steps(
            processing_order, prompts_dir, repo_structure, config_overrides, deps_formatted_content
        )
