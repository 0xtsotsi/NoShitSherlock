...
class InvestigateReposWorkflow:
    ...
    @workflow.run
    async def run(
        self, request: InvestigateReposRequest | None = None, iteration_count: int = 0
    ) -> InvestigateReposResult:
        ...
        if request:
            ...
            if force_first_run and iteration_count == 0:
                logger.info(
                    "🚀 Force flag detected - will force investigation of all repositories on first run"
                )
            else:
                logger.info(
        ...
        workflow.continue_as_new(args=[next_request, iteration_count + 1])

        # This line should never be reached due to continue_as_new
        return result

    async def _run_investigation(
        self, force: bool = False, config_overrides: ConfigOverrides = None
    ) -> InvestigateReposResult:
        ...
        if force:
            logger.info(
                "⚡ Force mode enabled - all repositories will be investigated regardless of cache"
            )

        # Process repositories in chunks - only specified number run in parallel at a time
        window_size = (
            config_overrides.chunk_size or WorkflowConfig.WORKFLOW_CHUNK_SIZE
        ...
        for chunk_idx in range(0, len(valid_repos), window_size):
            ...
            for repo_name, repo_info in chunk:
                ...
                chunk_info_map[handle] = {"repo_name": repo_name, "repo_url": repo_url}
            ...
            for handle in chunk_handles:
                ...
                try:
                    ...
                    error_result = InvestigateSingleRepoResult(
                        ...
                        branch_name="unknown",
                        reason=f"Failed to execute investigation: {e!s}",
                        message=f"Failed to execute investigation for {repo_info['repo_name']}: {e!s}",
                    )
        ...
        if success_count > 0:
            ...
            try:
                ...
                logger.info("Skipping architecture analysis - not implemented yet")
