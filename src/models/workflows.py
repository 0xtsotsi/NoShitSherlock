...
class ConfigOverrides(BaseModel):
    ...
    sleep_hours: float | None = Field(
        None, ge=0.1, le=168.0, description="Hours to sleep between executions"
    )
    chunk_size: int | None = Field(
        None, ge=1, le=100, description="Number of repos to process in parallel"
    )
    force_section: str | None = Field(
        None, description="Force re-execution of specific section (prompt name)"
    )
...
class InvestigateSingleRepoRequest(BaseModel):
    ...
    repo_type: str | None = Field(default="generic", description="Repository type")
    force: bool = Field(default=False, description="Force investigation even if cached")
    config_overrides: ConfigOverrides | None = Field(None, description="Configuration overrides")
...
class AnalysisStepResult(BaseModel):
    ...
    cache_reason: str | None = Field(None, description="Reason for cache hit/miss")
...
class InvestigateSingleRepoResult(BaseModel):
    ...
    arch_file_path: str | None = Field(None, description="Path to architecture file")
    analysis_steps: int = Field(default=0, ge=0, description="Number of analysis steps completed")
    prompt_versions: dict[str, str] = Field(
        default_factory=dict, description="Versions of prompts used"
    )
    ...
    arch_file_content: str | None = Field(None, description="Content of the architecture file")
    architecture_hub: dict[str, Any] | None = Field(
        None, description="Architecture hub save result"
    )
    metadata_saved: dict[str, Any] | None = Field(None, description="Metadata save result")
    cleanup: dict[str, Any] | None = Field(None, description="Cleanup operation result")
    last_investigation_timestamp: str | None = Field(
        None, description="Timestamp of last investigation"
    )
...
class InvestigationResult(BaseModel):
    ...
    @field_validator("arch_file_content")
    @classmethod
...
class InvestigateReposRequest(BaseModel):
    ...
    force: bool = Field(
        default=False, description="Force investigation of all repos ignoring cache"
    )
    claude_model: str | None = Field(None, description="Override the Claude model to use")
    max_tokens: int | None = Field(None, ge=1, le=200000, description="Override the max tokens")
    sleep_hours: float | None = Field(
        None, ge=0.1, le=168.0, description="Hours to sleep between executions"
    )
    chunk_size: int | None = Field(
...
class InvestigateReposResult(BaseModel):
    ...
    investigated_repos: list[InvestigateSingleRepoResult] = Field(
        ..., description="Results from all repository investigations"
    )
    repository_update: dict[str, Any] = Field(..., description="Repository list update results")
    architecture_analysis: dict[str, Any] | None = Field(
        None, description="Architecture analysis results"
    )
    ...
    @classmethod
    def validate_successful(cls, v, info):
        """Ensure successful count doesn't exceed total."""
        total_repos = info.data.get("total_repos")
        if total_repos is not None and v > total_repos:
            raise ValueError("Successful count cannot exceed total repositories")
        return v

    @field_validator("failed")
    @classmethod
    def validate_failed(cls, v, info):
    ...
    @classmethod
    def validate_skipped(cls, v, info):
...
class WorkflowParams(BaseModel):
    ...
    repo_name: str = Field(..., description="Name of the repository")
    repo_url: str = Field(..., description="URL of the repository")
    repo_path: str | None = Field(None, description="Local path to repository if already cloned")
    force_investigation: bool = Field(
        default=False, description="Force investigation even if cached"
    )
    prompt_versions: dict[str, str] | None = Field(
        None, description="Mapping of prompt names to versions"
    )
    max_retries: int = Field(
        default=3, ge=1, le=10, description="Maximum number of retries for activities"
    )
    timeout_minutes: int = Field(
        default=30, ge=5, le=120, description="Overall workflow timeout in minutes"
    )
...
class AnalysisSummary(BaseModel):
    ...
    sections_analyzed: list[str] = Field(
        default_factory=list, description="List of sections analyzed"
    )

    @field_validator("successful_prompts")
    @classmethod
    def validate_successful_prompts(cls, v, info):
        """Ensure successful prompts doesn't exceed total."""
        total_prompts = info.data.get("total_prompts")
        if total_prompts is not None and v > total_prompts:
            raise ValueError("Successful prompts cannot exceed total prompts")
        return v

    @field_validator("failed_prompts")
    @classmethod
    def validate_failed_prompts(cls, v, info):
...
class RepositoryAnalysis(BaseModel):
    ...
    prompt_versions: dict[str, str] | None = Field(None, description="Versions of prompts used")

    @field_validator("analysis_content")
    @classmethod
...
class WorkflowResult(BaseModel):
    ...
    analysis: RepositoryAnalysis | None = Field(
        None, description="Analysis results if investigation was performed"
    )
