...
try:
    ...
    # This fallback path may not find modules, ignore errors
    from activity_wrapper import ActivityWrapper  # type: ignore
    ...
    from core.claude_analyzer import ClaudeAnalyzer  # type: ignore
...
class ClaudeInvestigator:
    ...
    def _setup_logging(self, log_level: str):
        ...
        if not logging.getLogger().handlers:
            ...
            logging.basicConfig(level=level, handlers=[console_handler], format=Config.LOG_FORMAT)
        else:
    ...
    # ---------------------------------
    @staticmethod
    def _heartbeat_safe(details: str | None = None) -> None:
        ...
        try:
            ...
            print(f"Failed to heartbeat: {e!s}")
    ...
    async def investigate_repository(self, repo_location: str, repo_type: str | None = None) -> str:
        """
    ...
    def _build_context_from_config(
        self, context_config: dict | list[dict] | None, step_results: dict[str, str]
    ) -> str:
        """
    ...
    async def _process_analysis_step(
        self, step: dict, prompts_dir: str, repo_structure: str, step_results: dict[str, str]
    ) -> dict | None:
        """
        ...
        try:
            ...
            if self.temp_dir is None:
                raise ValueError("temp_dir must be set before processing analysis steps")
