...
class Config:
    ...
    # Claude CLI settings (for subscription-based usage via subclaude technique)
    # When True, uses Claude CLI instead of API (requires authenticated CLI)
    USE_CLAUDE_CLI: bool = os.getenv("USE_CLAUDE_CLI", "false").lower() == "true"
    CLAUDE_CLI_TIMEOUT: int = int(os.getenv("CLAUDE_CLI_TIMEOUT", "300"))  # 5 minutes default

    # Valid Claude model names for validation (4.x models only)
    # See: https://platform.claude.com/docs/en/about-claude/models/overview
    VALID_CLAUDE_MODELS: ClassVar[list[str]] = [
        # Claude 4.5 (current)
    ...
    SIZE_UNITS: ClassVar[list[str]] = ["B", "KB", "MB", "GB", "TB"]

    # Architecture Hub configuration
    # These values are read from environment variables with sensible defaults
    ARCH_HUB_REPO_NAME: str = os.getenv("ARCH_HUB_REPO_NAME", "architecture-hub")
    ARCH_HUB_BASE_URL: str = os.getenv("ARCH_HUB_BASE_URL", "https://github.com/your-org")
    ARCH_HUB_FILES_DIR: str = os.getenv(
        "ARCH_HUB_FILES_DIR", ""
    )  # Empty string means root directory

    # Repository scanning configuration
    # DEFAULT_ORG_NAME supports both GitHub organizations and individual user accounts
    DEFAULT_ORG_NAME: str = os.getenv("DEFAULT_ORG_NAME", "your-org")
    DEFAULT_REPO_URL: str = os.getenv("DEFAULT_REPO_URL", "https://github.com/facebook/react")

    # Git configuration for commits
    GIT_USER_NAME: str = os.getenv("GIT_USER_NAME", "Architecture Bot")
    GIT_USER_EMAIL: str = os.getenv("GIT_USER_EMAIL", "architecture-bot@your-org.com")
    ...
    WORKFLOW_CHUNK_SIZE: int = 8  # Number of sub-workflows to run in parallel
    WORKFLOW_SLEEP_HOURS: int = 6  # Hours to sleep between workflow executions
