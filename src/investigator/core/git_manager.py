...
class GitRepositoryManager:
        ...
        try:
            ...
            if isinstance(e, git.exc.GitCommandError):
                ...
                if "exit code(-9)" in str(e) or "Killed" in str(e):
                    self.logger.warning(
                        "Detected potential resource issue, attempting minimal clone"
                    )
                    # Clean up failed attempt
                    if os.path.exists(target_dir):
                        shutil.rmtree(target_dir, ignore_errors=True)

                    # Try minimal clone as fallback
                    try:
    ...
    def _shallow_clone_fallback(self, repo_location: str, target_dir: str) -> str:
        ...
        try:
            ...
            raise Exception(f"Shallow clone failed: {error_msg}") from e
        except subprocess.TimeoutExpired:
            raise Exception("Shallow clone timed out after 10 minutes") from None
    ...
    def _minimal_clone_fallback(self, repo_location: str, target_dir: str) -> str:
        ...
        try:
            ...
            subprocess.run(
                ["git", "config", "core.packedGitLimit", "128m"], cwd=target_dir, check=True
            )
            subprocess.run(
                ["git", "config", "core.packedGitWindowSize", "128m"], cwd=target_dir, check=True
            )
            ...
            subprocess.run(
                fetch_cmd, cwd=target_dir, capture_output=True, text=True, timeout=600, check=True
            )
            ...
            raise Exception(f"Minimal clone failed: {error_msg}") from e
        except subprocess.TimeoutExpired:
            raise Exception("Minimal clone timed out after 10 minutes") from None
    ...
    def validate_github_token(self) -> dict:
        ...
        """
        if not self.github_token:
            return {"status": "no_token", "message": "No GitHub token found in environment"}
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Could not validate GitHub token: {e!s}",
                "error": str(e),
            }
    ...
    def check_repository_permissions(self, repo_url: str) -> dict:
        ...
        try:
            if "github.com" not in repo_url:
                return {"status": "not_github", "message": "Repository is not hosted on GitHub"}
            ...
            return {
                "status": "error",
                "message": f"Failed to check repository permissions: {e!s}",
                "error": str(e),
            }
