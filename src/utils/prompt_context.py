...
def create_prompt_context(
    repo_name: str, step_name: str, prompt_version: str = "1"
) -> PromptContextBase:
    """
...
def create_prompt_context_from_dict(data: dict[str, Any]) -> PromptContextBase:
    ...
    if backend == "dynamodb":
        logger.debug("Creating DynamoDBPromptContext from dict")
        return DynamoDBPromptContext.from_dict(data)
    else:
        logger.debug("Creating FileBasedPromptContext from dict")
...
def prompt_context_create_for_step(repo_name: str, step_name: str) -> PromptContextBase:
    """Backward compatibility wrapper for create_prompt_context."""
    return create_prompt_context(repo_name, step_name)


def prompt_context_from_dict(data: dict[str, Any]) -> PromptContextBase:
