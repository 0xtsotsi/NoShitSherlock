...
class FileBasedPromptContext(PromptContextBase):
    ...
    def __post_init__(self):
        ...
        base_dir_str: str | None = os.environ.get("PROMPT_CONTEXT_STORAGE_DIR")
        if base_dir_str:
            ...
            project_root = Path(
                __file__
            ).parent.parent.parent  # Go up from src/utils/ to project root
    ...
    def _get_file_path(self, key: str) -> Path:
        ...
        if self._storage_dir is None:
            raise RuntimeError("_storage_dir is not initialized")
    ...
    def save_prompt_data(
        ...
        """
        _ = ttl_minutes  # TTL is ignored in file implementation
        # Generate unique reference key using KeyNameCreator
        unique_id = str(uuid.uuid4())[:8]
        if self.step_name is None:
            raise ValueError("step_name must be set before saving prompt data")
        key_obj = KeyNameCreator.create_prompt_data_key(
            repo_name=self.repo_name, step_name=self.step_name, unique_id=unique_id
        )
...
class FileBasedPromptContextManager(PromptContextManagerBase):
    ...
    def create_context_for_step(
        ...
        # The return type is FileBasedPromptContext (subclass of PromptContextBase)
    ...
    def save_investigation_metadata(
        ...
        Returns:
            Dictionary with save status
        """
        _ = ttl_days  # TTL is ignored in file implementation
        try:
            # Use KeyNameCreator to generate file-safe key
            metadata_key_obj = KeyNameCreator.create_investigation_metadata_key(
                repo_name=repository_name, analysis_type=analysis_type
            )
    ...
    def get_latest_investigation(self, repository_name: str) -> dict[str, Any] | None:
        ...
        try:
            # Use KeyNameCreator to generate file-safe key
            metadata_key_obj = KeyNameCreator.create_investigation_metadata_key(
                repo_name=repository_name, analysis_type="investigation"
            )
            file_safe_key = metadata_key_obj.to_file_safe_key()
