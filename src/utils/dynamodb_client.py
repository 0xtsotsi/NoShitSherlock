...
class DynamoDBClient:
    ...
    def __init__(self, table_name: str | None = None):
        ...
        if table_name:
            ...
            table_name_from_ssm = self._get_table_name_from_ssm()
            if table_name_from_ssm:
                self.table_name = table_name_from_ssm
            else:
    ...
    def _get_table_name_from_ssm(self) -> str | None:
        ...
        try:
            ...
            ssm_client = boto3.client(
                "ssm", region_name=os.environ.get("AWS_DEFAULT_REGION", "us-east-1")
            )
    ...
    def get_latest_analysis(
        self, repository_name: str, analysis_type: str | None = None
    ) -> dict[str, Any] | None:
        """
    ...
    def save_temporary_analysis_data(
        ...
        try:
            ...
            # Prepare the data
            data_to_store = {"prompt_content": prompt_content, "repo_structure": repo_structure}
            if context:
    ...
    def _get_chunked_analysis_data(
        self, reference_key: str, total_chunks: int
    ) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            for i in range(total_chunks):
                response = self.table.query(
                    KeyConditionExpression=Key("repository_name").eq(
                        f"_temp_{reference_key}_chunk_{i}"
                    ),
        ...
        try:
            ...
            if items:
                ...
                if item.get("is_chunked", False):
                    return self._get_chunked_analysis_data(
                        reference_key, item.get("total_chunks", 0)
                    )
    ...
    def get_multiple_analysis_data(self, reference_keys: list) -> dict[str, Any]:
        ...
        for key in reference_keys:
            try:
                ...
                if data:
                    results[key] = {"type": "analysis_data", "data": data}
                    continue
...
def get_dynamodb_client() -> DynamoDBClient:
    ...
    """
    global _dynamodb_client  # noqa: PLW0603
