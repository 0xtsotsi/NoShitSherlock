...
@activity.defn
async def check_if_repo_needs_investigation(input_params: CacheCheckInput) -> CacheCheckOutput:
    """
    ...
    try:
        ...
        activity.logger.info(
            f"📊 Repository state: commit={current_state.commit_sha[:8]}, branch={current_state.branch_name}, uncommitted={current_state.has_uncommitted_changes}"
        )

        # Get DynamoDB client and create cache instance
        activity.logger.info("🗃️  Creating DynamoDB client and cache instance")
        ...
        activity.logger.info("🔍 Calling cache.check_needs_investigation...")
        decision = cache.check_needs_investigation(
            input_params.repo_name, current_state, input_params.prompt_versions
        )

        # Convert decision to CacheCheckOutput model
        activity.logger.info(
            f"✅ ACTIVITY RESULT: needs_investigation={decision.needs_investigation}, reason='{decision.reason}'"
        )
...
@activity.defn
async def save_investigation_metadata(input_params: SaveMetadataInput) -> SaveMetadataOutput:
    """
    ...
    try:
        ...
        activity.logger.info("💾 Calling cache.save_investigation_metadata...")
        ...
        activity.logger.info(
            f"✅ ACTIVITY RESULT: save_investigation_metadata status={result.get('status')}"
        )
