...
@activity.defn
async def update_repos_list() -> dict[str, Any]:
    """
...
@activity.defn
async def read_repos_config() -> dict[str, Any]:
    """
...
def _read_arch_file_content(arch_file_path: str) -> str:
    ...
    try:
        if os.path.exists(arch_file_path):
            with open(arch_file_path, encoding="utf-8") as f:
                return f.read()
        else:
            return ""
    except OSError as e:
...
@activity.defn
async def save_to_arch_hub(arch_files: list[dict[str, Any]]) -> dict[str, Any]:
    """
...
async def save_prompt_context_activity(
    ...
    prompt_content: str,
    repo_structure: str,
    deps_formatted_content: str | None = None,
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if deps_formatted_content:
            ...
            needs_deps = any(
                keyword.lower() in prompt_content.lower() for keyword in dependency_keywords
            )

            if needs_deps:
                activity.logger.info("Prompt contains dependency keywords - including dependencies")
                ...
                activity.logger.debug(
                    "Prompt does not contain dependency keywords - skipping dependencies"
                )
        else:
            # Replace with "not found" message if prompt expects dependencies
            if "{repo_deps}" in prompt_content:
                activity.logger.info("Prompt expects dependencies but none were provided")
...
@activity.defn
async def retrieve_all_results_activity(manager_dict: dict[str, Any]) -> dict[str, Any]:
    """
...
@activity.defn
async def cleanup_temporary_analysis_data_activity(reference_key: str) -> dict[str, Any]:
    """
...
async def analyze_with_claude_context(
    ...
    config_overrides = (
        input_params.config_overrides.model_dump() if input_params.config_overrides else {}
    )
    ...
    try:
        ...
        if latest_commit and repo_name and step_name:
            ...
            force_section = (
                getattr(config_overrides, "force_section", None) if config_overrides else None
            )
            ...
            if cache_check["cached_result"]:
                ...
                # Ensure cached_result is a string for len()
                result_length = len(cached_result) if isinstance(cached_result, str) else 0
        ...
        activity.logger.info("Successfully prepared data for Claude analysis")
...
@activity.defn
async def clone_repository_activity(repo_url: str, repo_name: str) -> dict[str, Any]:
    """
...
def _minimal_clone_repository(repo_url: str, target_dir: str, logger=None) -> str:
    ...
    result = subprocess.run(
        fetch_cmd, check=False, cwd=target_dir, capture_output=True, text=True, timeout=600
    )
...
@activity.defn
async def analyze_repository_structure_activity(repo_path: str) -> dict[str, Any]:
    """
...
@activity.defn
async def read_prompt_file_activity(prompts_dir: str, file_name: str) -> dict[str, Any]:
    """
...
@activity.defn
async def cleanup_repository_activity(
    repo_path: str, temp_dir: str | None = None
) -> dict[str, Any]:
    """
...
@activity.defn
async def write_analysis_result_activity(repo_path: str, final_analysis: str) -> dict[str, Any]:
    """
...
@activity.defn
async def read_dependencies_activity(repo_path: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        dependency_patterns = {
            ...
            "Rust": {"production": ["Cargo.toml"], "dev": [], "exclude": ["Cargo.lock"]},
            "Java": {
                "production": [
                    "pom.xml",
                    "build.gradle",
            ...
            "PHP": {"production": ["composer.json"], "dev": [], "exclude": ["composer.lock"]},
            "Other": {
                "production": [
                    "Dockerfile",
                    "docker-compose.yml",
                ...
                for file_path in found_files:
                    ...
                    try:
                            ...
                            try:
                                ...
                                if prod_content.strip():
                                    production_deps.append(
                                        {"full_path": relative_path, "content": prod_content}
                                    )
                        ...
                        # For Python pyproject.toml, parse sections
                            ...
                            if dev_content.strip():
                                dev_deps.append(
                                    {"full_path": relative_path + " (dev)", "content": dev_content}
                                )
            ...
            for pattern in patterns["dev"]:
                ...
                for file_path in found_files:
                    try:
                        ...
                        relative_path = "/" + str(file_path.relative_to(repo_path_obj))
                        dev_deps.append({"full_path": relative_path, "content": content})
...
def _format_dependencies_for_prompt(dependencies: dict[str, Any]) -> str:
...
def _extract_package_json_section(content: str, sections: list[str]) -> str:
...
def _parse_gemfile_groups(content: str) -> tuple[str, str]:
...
def _parse_pyproject_dependencies(content: str) -> tuple[str, str]:
...
@activity.defn
async def cache_dependencies_activity(
    repo_name: str, dependencies_data: dict[str, Any]
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        result = cache.save_dependencies(
            repo_name=repo_name, dependencies_data=dependencies_data, reference_key=reference_key
        )
