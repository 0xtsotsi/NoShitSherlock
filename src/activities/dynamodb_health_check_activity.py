...
async def check_dynamodb_health() -> dict:
    ...
    try:
        ...
        response = dynamodb_client.table.get_item(
            Key={"repository_name": test_repo_name, "analysis_timestamp": test_timestamp}
        )
        ...
        if read_item.get("repository_name") != test_repo_name:
            raise Exception(
                f"Read verification failed - expected {test_repo_name}, got {read_item.get('repository_name')}"
            )
        ...
        if "Item" in verify_response:
            activity.logger.warning(
                "Test item still exists after deletion (may be eventual consistency)"
            )
        else:
